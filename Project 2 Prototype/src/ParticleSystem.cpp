...
void GravityForce::setForce(const ofVec3f& g) {
	gravity = g;
}
...
void TurbulenceForce::setForce(const ofVec3f& min, const ofVec3f& max) {
	tmin = min;
	tmax = max;
}
...
void ImpulseRadialForce::setForce(float magnitude) {
	this->magnitude = magnitude;
}
