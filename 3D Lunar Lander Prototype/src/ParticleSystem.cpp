...
// Edited by Nate Agpaoa - CS 134 SJSU
...
// I tried to replicate the thrust emitter with this force
CylinderForce::CylinderForce(float magnitude) {
	this->magnitude = magnitude;
}

void CylinderForce::updateForce(Particle* particle) {
	ofVec3f dir = ofVec3f(ofRandom(-1,1), ofRandom(-100, -1), ofRandom(-1,1));
	particle->forces += dir.getNormalized() * magnitude;
}