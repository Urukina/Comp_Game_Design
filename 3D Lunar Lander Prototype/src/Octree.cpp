...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
	...
	for (int i = 0; i < bList.size(); i++) {
		...
		if (child.points.size() > 0) node.children.push_back(child);
		if (child.points.size() > 1) subdivide(mesh, node.children[node.children.size() - 1], numLevels, level++);
	}
}
...
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn) {
	...
	for (int i = 0; i < node.children.size(); i++) {
		if (node.children[i].box.intersect(ray, -1000, 1000)) return intersect(ray, node.children[i], nodeRtn);
	}
	return intersects;
}
...
bool Octree::intersect(const Box &box, TreeNode & node, vector<Box> & boxListRtn) {
	...
	for (int i = 0; i < node.children.size(); i++) {
		if (node.children[i].box.overlap(box)) return intersect(box, node.children[i], boxListRtn);
	}
	return intersects;
}
